resource "google_compute_global_address" "pg" {
  name          = "factorymind-${var.environment}-pg-psa"
  purpose       = "VPC_PEERING"
...
resource "google_sql_user" "factorymind" {
  name     = "factorymind"
  instance = google_sql_database_instance.postgres.name
  password = random_password.pg.result
}

resource "google_sql_database" "factorymind" {
  name     = "factorymind"
  instance = google_sql_database_instance.postgres.name
}
...
# Pub/Sub used as the Kafka equivalent on GCP for cross-domain events.
resource "google_pubsub_topic" "audit" {
  name = "factorymind-${var.environment}-audit"
}