# FactoryMind â€” top-level Makefile
...
help:
	@echo "FactoryMind â€” common targets"
	@echo ""
...
tools-deploy:  ## Deploy every OSS tool registered for FactoryMind via ArgoCD
	kubectl apply -f gitops/argocd/applicationsets/all-tools-applicationset.yaml
	argocd appset get factorymind-tools --grpc-web

tools-status:  ## Show sync status of every registered tool
	argocd app list -l "project=factorymind-tools" --grpc-web
